...
app.get('/api/user-bookings', async (req, res) => {
    try {
        ...
        const bookings = await db.collection('bookings').find({ user_id: userId }).sort({ booking_date: -1 }).toArray();

        const result = [];
        for (const b of bookings) {
            // Get flight details
            const flight = await db.collection('flights').findOne({ id: b.flight_id });

            // Get passengers for this booking
            const passengers = await db.collection('passengers').find({ booking_id: b.id }).toArray();

            // Get cab booking if any
            const cab = await db.collection('cab_bookings').findOne({ booking_id: b.id });

            // Get hotel booking if any
            const hotel = await db.collection('hotel_bookings').findOne({ booking_id: b.id });
            ...
            result.push({
                ...
                cab: cab ? {
                    pickup: cab.pickup_location,
                    drop: cab.drop_location,
                    cabType: cab.cab_type,
                    amount: cab.amount,
                    isPooling: cab.is_pooling === 1
                } : null,
                hotel: hotel ? {
                    hotelName: hotel.hotel_name,
                    roomType: hotel.room_type,
                    checkin: hotel.checkin_date,
                    checkout: hotel.checkout_date,
                    amount: hotel.amount
                } : null
            });
        }
        ...
        console.error(e);
        res.json({ success: false, message: 'Error fetching bookings: ' + e.message });
    }
});
...
async function clearBookingsData() {
    console.log('=== Clearing bookings/earnings data ===');
    try { await db.collection('bookings').deleteMany({}); } catch(e) {}
    try { await db.collection('passengers').deleteMany({}); } catch(e) {}
    try { await db.collection('booked_seats').deleteMany({}); } catch(e) {}
    try { await db.collection('cab_bookings').deleteMany({}); } catch(e) {}
    try { await db.collection('hotel_bookings').deleteMany({}); } catch(e) {}

    // Reset counters for booking-related collections
    const resetCounters = ['bookings', 'passengers', 'cab_bookings', 'hotel_bookings'];
    for (const name of resetCounters) {
        try {
            await db.collection('counters').updateOne({ _id: name }, { $set: { seq: 0 } });
        } catch(e) {}
    }
    console.log('=== Bookings/earnings data cleared ===');
}
