...
app.get('/FlightServlet', async (req, res) => {
    try {
        ...
        for (const cf of connectingFlights) {
            ...
            const discount = cf.discount_percent;
            const finalPrice = Math.round(combinedBase * (1 - discount / 100) * dateMultiplier);
...
async function initializeData() {
    ...
    const counterValues = { users: 2, flights: 27, bookings: 0, passengers: 0, cab_bookings: 0, hotel_bookings: 0, connecting_flights: 2 };
    ...
    await db.collection('connecting_flights').insertMany([
        { id: 1, route_name: 'Delhi-Chennai via Bangalore', flight1_id: 12, flight2_id: 17, layover_minutes: 75, discount_percent: 10 },
        { id: 2, route_name: 'Delhi-Chennai via Mumbai', flight1_id: 1, flight2_id: 7, layover_minutes: 120, discount_percent: 8 }
    ]);

    const count = await db.collection('flights').countDocuments();
    console.log('=== Seeded ' + count + ' flights, 2 users, 2 connecting routes ===');
}
